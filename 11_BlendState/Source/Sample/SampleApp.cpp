    ...
    {
        {
            ...
            {
                color.a = 1.0f - i * 0.5f;
                m_Plane[i] = std::make_unique<Shape>(
                    m_Device,
                    m_Context
                    );
                m_Plane[i]->InitializeAsPlane(1.0f, 1.0f, &color);
            }
        }
        ...
        // 平行光設定
        {
            ...
            for (u8 i = 0; i < 2; ++i)
            {
                m_Plane[i]->SetLightPosition(light);
            }
        }
        ...
        // 環境光設定
        {
            ...
            for (u8 i = 0; i < 2; ++i)
            {
                m_Plane[i]->SetAmbientColor(ambientColor);
            }
        }
        ...
        // 視線ベクトル & ビュー行列 設定
        {
            ...
            for (u8 i = 0; i < 2; ++i)
            {
                m_Plane[i]->SetEyePosition(glm::vec4(eye.x, eye.y, eye.z, 1.0f));
                m_Plane[i]->SetViewMatrix(view);
            }
        }
        ...
        // プロジェクション行列設定
        {
            ...
            for (u8 i = 0; i < 2; ++i)
            {
                m_Plane[i]->SetProjectionMatrix(projection);
            }
        }
    }
...
void SampleApp::Update()
{
...

    {
        ...
        for (u8 i = 0; i < 2; ++i)
        {
            translate.z = (float)i;
            translate.x = i * 0.5f;
            m_Plane[i]->Update(&translate, nullptr, &scale);
        }
    }
    ...
    if (m_ImGuiActive)
    {
        ...
        ImGui::RadioButton("Plane", &m_VisibleObject, 4);
...
void SampleApp::Render()
{
    ...
    {
        m_Cylinder->Draw(false);
        //m_Cylinder->Draw(true);
    }
    if (m_VisibleObject == 4)
    {
        for (u8 i = 0; i < 2; ++i)
        {
            m_Plane[i]->Draw(false);
        }
    }
