...
// 板ポリゴンとして初期化
void Shape::InitializeAsPlane(f32 scaleX, f32 scaleY, const glm::vec4* pColor)
{
    ...
    */
    ...
    for (u16 i = 0; i < 4; ++i)
    {
        ...
        // 頂点座標
        v.Position.x = 0.5f * ((i / 2) ? 1.0f : -1.0f) * scaleX;
        v.Position.y = 0.5f * ((i % 2) ? -1.0f : 1.0f) * scaleY;
        v.Position.z = 0.0f;
        v.Position.w = 1.0f;
        ...
        {
            glm::vec3 color = glm::rgbColor(glm::vec3(360.0f / 4 * i, 1.0f, 1.0f));
            v.Color.x = color.x;
            v.Color.y = color.y;
            v.Color.z = color.z;
            v.Color.w = 1.0f;
        ...
        // 法線
        v.Normal = glm::vec4(0.0f, 0.0f, 1.0f, 1.0f);

        // UV座標
        v.Texture.x = 1.0f - (float)(i / 2);
        v.Texture.y = (float)(i % 2);
...
void Shape::Draw(bool outline)
{
    ...
    {
        // プリミティブトポロジーの設定
        m_Context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST_ADJ);
...
)
{
    ...
    // シェーダー
    {
        ...
        m_OutlineShader->Initialize(
            "Resource\\Shader\\bin\\OutlineVS.cso",
            "Resource\\Shader\\bin\\OutlinePS.cso",
            "Resource\\Shader\\bin\\OutlineGS.cso"
        );
    }
